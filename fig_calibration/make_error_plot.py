...
def plot_ecdf(ax, data, *, weights=None, swapaxes=False, **kwargs):
    ...
    if not swapaxes:
        return ax.plot([data[0], *data], [0, *cweights],
                       drawstyle="steps-post", **kwargs)[0]
    else:
        return ax.plot([0, *cweights], [data[0], *data],
                       drawstyle="steps-pre", **kwargs)[0]
...
def plot_eccdf(ax, data, *, weights=None, swapaxes=False, **kwargs):
    ...
    if not swapaxes:
        return ax.plot(
            [*data, data[-1]], [1, *1 - cweights],
            drawstyle="steps-pre", **kwargs)[0]
    else:
        return ax.plot(
            [1, *1 - cweights], [*data, data[-1]],
            drawstyle="steps-post", **kwargs)[0]
...
def make_ecdf(filenames, out_name, remove_all_labels):
    ...
    for path in filenames:
        ...
        plot_ecdf(ax, errors,swapaxes=False, label=label,
                  color='black' if 'synth' in name else None)
...
if __name__ == "__main__":
    ...
    make_ecdf(reversed(sorted(glob.glob(os.path.join("data", "*.txt")))), "all.svg", remove_all_labels=True)
    make_ecdf(reversed(sorted(glob.glob(os.path.join("data", "*.txt")))), "withlabels_all.svg", remove_all_labels=False)
